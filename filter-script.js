...
var replace_capitalized = "Pro-life";

for (var i=0; i<textNodes.snapshotLength;i++){
	var node = textNodes.snapshotItem(i);
	node.data = node.data.replace(RE, replace_capitalized);
}

//The below logic is for parsing the language that crosses nodes, which will primarily be seen in titles and search results, so assume caps is ok
